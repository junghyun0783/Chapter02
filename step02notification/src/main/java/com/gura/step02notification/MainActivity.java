...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void makeNoti(boolean isAutoCancel){
        ...
        // 새로운 Task 에서 엑티비티를 실행할수 있도록 플레그 설정
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        // NotiActivity 에 전달할 데이터가 있다면 intent 객체에 담는다.
        intent.putExtra("msg","Intent 객체에 전달한 데이터입니다.");
        ...
        Notification noti=new NotificationCompat.Builder(this)
                ...
                .setDefaults(Notification.DEFAULT_SOUND |
                        Notification.DEFAULT_VIBRATE)  //기본 알림(진동,소리)
                .setAutoCancel(isAutoCancel)  //한번 클릭하면 자동으로 사라진다.
                .build();
        //알림 메니져 객체의 참조값을 얻어와서
        NotificationManager notiManager=(NotificationManager)
                getSystemService(Context.NOTIFICATION_SERVICE);
        //알림을 띄운다
        notiManager.notify(AppConstants.NOTI_ID,noti);  //(알림의 아이디,알림 객체)
    }
}
